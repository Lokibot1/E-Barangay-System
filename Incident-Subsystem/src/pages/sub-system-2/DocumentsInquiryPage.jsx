    ...
    <div className="flex items-center gap-1.5">
      ...
      <button
        ...
        className={`w-7 h-7 rounded-lg flex items-center justify-center ${
          isDark ? "bg-slate-600 text-slate-200 hover:bg-slate-500" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
        }`}
      >
        👁
      </button>
  ...
  return (
    <div className={`min-h-full ${t.pageBg} pb-10`}>
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        <h1 className={`text-3xl font-bold ${t.cardText}`}>Issuance Application</h1>
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-semibold border transition ${
                  isActive ? `${config.bg} ${config.text} ${config.border}` : isDark ? "bg-slate-700 text-white border-slate-600" : "bg-gray-100 text-gray-600 border-gray-200"
                }`}
              >
                {tab}
              </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {paginatedCards.map((card) => (
            <div key={card.reference_number} className={`${t.cardBg} border ${t.cardBorder} rounded-xl p-5 shadow-md`}>
              <div className="flex justify-between items-start mb-3">
                <div>
                  <h3 className={`font-bold text-lg ${t.cardText}`}>{card.full_name}</h3>
              ...
              <div className="space-y-1 text-sm">
                <p><strong>Document:</strong> {card.documentType}</p>
                <p><strong>Contact:</strong> {card.contact_number}</p>
                <p><strong>Submitted:</strong> {new Date(card.dateSubmitted).toLocaleDateString()}</p>
              </div>
                ...
                <IconActionButtons
                  ...
                  onStatusUpdated={(refNum, newStatus) => {
                    setDocumentCardsData((prev) =>
                      prev.map((doc) => (doc.reference_number === refNum ? { ...doc, status: newStatus } : doc))
                    );
                  }}
                />
              </div>
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div className={`${t.cardBg} rounded-xl p-6 w-[400px] shadow-xl`}>
              ...
              <div className="space-y-2 text-sm">
                <p><strong>Name:</strong> {previewData.full_name}</p>
              ...
              <div className="flex justify-end mt-5">
                <button onClick={() => setShowPreview(false)} className="px-4 py-2 rounded-lg bg-gray-200 hover:bg-gray-300">
                  Close
                </button>
              </div>
            </div>
