...
const Step2Address = ({
  ...
  const getDateValidationState = () => {
    if (!formData.residencyStartDate || !formData.residencyStatus)
      return "empty";
    const selected = new Date(formData.residencyStartDate);
    if (selected > today) return "future";
    if (formData.residencyStatus === "New Resident")
      return selected >= sixMonthsAgo ? "valid" : "mismatch_new";
    if (formData.residencyStatus === "Old Resident")
      return selected < sixMonthsAgo ? "valid" : "mismatch_old";
    return "valid";
  };
  ...
  const isAddressComplete =
    formData.houseNumber?.trim() && formData.street && formData.purok;
  ...
  const disabledReason = useMemo(() => {
    ...
    if (dateState !== "valid")
      return `Date error: ${dateErrorMessages[dateState]}`;
    ...
    <div className="space-y-4 animate-in fade-in slide-in-from-right-3 duration-300">
      ...
      <div className="relative group">
        <label className={labelClass}>
          Search Registered Household Address
        </label>
        ...
        <p
          className={`text-[9px] font-bold mt-1 ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}
        >
        ...
        {addressSuggestions.length > 0 && (
          ...
          >
            {addressSuggestions.map((addr, idx) => (
              ...
              >
                <div
                  className={`p-2 rounded-lg ${selectedIndex === idx ? "bg-emerald-500 text-white" : "bg-slate-100 dark:bg-slate-800 text-emerald-500"}`}
                >
                  <MapPin size={14} />
                </div>
                <div>
      ...
      <div
        className={`h-px my-2 ${isDarkMode ? "bg-slate-800" : "bg-slate-100"}`}
      />
          ...
          {addressExists && isAddressComplete && (
            <div className="flex items-start gap-2 mt-1.5 p-2.5 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-200 dark:border-amber-700">
              ...
              <p className="text-[9px] font-bold text-amber-700 dark:text-amber-400 uppercase leading-tight">
                ...
                {householdHeadData && ` (${householdHeadData})`}. Please select
            ...
            !formData.householdPosition && (
              <div className="flex items-start gap-2 mt-1.5 p-2.5 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-200 dark:border-emerald-700">
                ...
                <p className="text-[9px] font-bold text-emerald-700 dark:text-emerald-400 uppercase leading-tight">
                  No existing household at this address. If this person owns or
                  leads the home, select "Head of Family".
                </p>
              </div>
            )}
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
        <div className="space-y-1">
          ...
          >
            ...
            <option value="Old Resident">
              Old Resident — living here 6+ months
            </option>
