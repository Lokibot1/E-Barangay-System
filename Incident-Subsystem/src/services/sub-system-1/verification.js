 ...
 updateStatus: async (id, status, isIndigent, additionalData) => {
  try {
    const response = await api.put(`/residents/${id}/status`, { 
      ...
      tenureStatus: additionalData?.tenureStatus,
      wallMaterial: additionalData?.wallMaterial,
      roofMaterial: additionalData?.roofMaterial,
      waterSource: additionalData?.waterSource,
      numberOfFamilies: additionalData?.numberOfFamilies,
