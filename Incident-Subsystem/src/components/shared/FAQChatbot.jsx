    ...
    <>
      {!isOpen && (
        <button
          ...
          className={`fixed bottom-7 sm:bottom-8 right-3 sm:right-4 z-[1550] inline-flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-full bg-gradient-to-br ${t.primaryGrad} text-white shadow-[0_18px_40px_rgba(15,23,42,0.22)] hover:-translate-y-0.5 transition-all duration-300 group border border-white/20`}
          aria-label="Open FAQ Chatbot"
        >
          <Bot className="w-5 h-5" strokeWidth={2.1} />
      ...
      {isOpen && (
        <div
          className={`fixed bottom-6 right-3 sm:bottom-7 sm:right-4 w-[calc(100vw-1.5rem)] sm:w-[20.5rem] z-[1550] ${shellSurface} rounded-[24px] shadow-[0_28px_80px_rgba(15,23,42,0.28)] border backdrop-blur-xl flex flex-col animate-slideUp font-kumbh overflow-hidden text-left`}
          style={{
        ...
        >
          ...
          >
            ...
            <button
              ...
              className={`p-1.5 rounded-xl border ${t.cardBorder} ${t.cardText} ${isDark ? "hover:bg-slate-800" : "hover:bg-slate-100"} transition-colors`}
            >
          ...
          <div
            className={`flex-1 overflow-y-auto p-3.5 sm:p-4 space-y-3.5 ${bodySurface}`}
          >
          ...
          {messages.length === 1 && (
            <div className={`px-4 py-3 border-t ${t.cardBorder} ${quickPanel} flex-shrink-0 backdrop-blur-xl`}>
              <p className={`text-[11px] ${t.subtleText} mb-2 font-kumbh font-medium flex items-center gap-1.5`}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {quickQuestions.map((question, index) => (
                  <button
                    ...
                    className={`text-[11px] px-3 py-1.5 border rounded-full transition-colors font-kumbh font-normal ${quickPill}`}
                  >
                    {question}
                  </button>
                ))}
          ...
          <div className={`p-3 border-t ${t.cardBorder} ${inputPanel} flex-shrink-0 backdrop-blur-xl`}>
            <div className="flex items-center space-x-2">
              <input
                ...
                className={`flex-1 px-3.5 py-2.5 border rounded-2xl ${inputField} focus:outline-none outline-none focus:ring-1 ring-offset-0 ${t.primaryRing} ${t.primaryBorder} transition-all text-[13px] font-kumbh`}
              />
              <button
                onClick={handleSend}
                disabled={!inputValue.trim()}
                className={`p-2.5 bg-gradient-to-br ${t.primaryGrad} text-white rounded-2xl hover:shadow-[0_12px_24px_rgba(15,23,42,0.18)] transition-all disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0`}
              >
