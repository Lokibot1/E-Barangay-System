...
const BIDRequestModal = ({ isOpen, onClose, currentTheme }) => {
	...
	const handleConfirmSubmit = async () => {
        ...
        // 1. Create FormData object (Standard way to handle files + text)
        ...
        if (uploadedFile) {
            // 'validIdFile' matches the $request->validate() key in your PHP
            data.append('validIdFile', uploadedFile);
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                queueCommunicationEvent({
                    category: "documents",
                    title: "Barangay ID request received",
                    message: `Your Barangay ID request ${savedRecord.reference} was submitted successfully.`,
                    metadata: {
                ...
                addToast({
                    ...
                    message: "Your application has been recorded successfully.",
                    duration: 5000,
                });
            } else {
                // Handle Laravel validation errors (422 Unprocessed Entity)
                addToast({ 
                    type: "error", 
                    title: "Submission Error", 
                    message: result.message || "Please check your inputs." 
                });
				...
				return (
					<>
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
							...
							<Field label="Emergency Contact No." name="emergencyContactNumber" value={formData.emergencyContactNumber} onChange={handleFieldChange} t={t} placeholder="09XXXXXXXXX" icon="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
						</div>
					</>
				);
			case 3:
				return (
					<>
						...
						<div className="grid grid-cols-1 large:grid-cols-2 gap-4 mb-6">
							<SelectField
								label="Blood Type"
								name="bloodType"
				...
				<div className={`${t.cardBg} border ${t.cardBorder} rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl`}>
					...
					<div className="flex items-center justify-between p-5 border-t border-gray-200/20 flex-shrink-0">
						...
						<button
							onClick={handleNext}
							disabled={isSubmitting}
							className="px-5 py-2.5 rounded-lg bg-blue-500 hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-60 text-white font-kumbh text-sm font-medium transition-colors"
						>
						...
						<div className={`${t.cardBg} ${t.cardBorder} border rounded-2xl w-full max-w-2xl p-6 max-h-[85vh] overflow-y-auto shadow-2xl`}>
							...
							<div className="mt-6 flex items-center justify-end gap-3">
								...
								<button
									...
									disabled={isSubmitting}
									className="px-5 py-2.5 rounded-lg bg-blue-500 hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-60 text-white font-kumbh text-sm font-medium"
								>
