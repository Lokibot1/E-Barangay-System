      ...
      case 2:
        return (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <SelectField label="Purok/Zone" name="purokZone" value={formData.purokZone} onChange={handleFieldChange} options={["Purok/Zone 1", "Purok/Zone 2", "Purok/Zone 3", "Purok/Zone 4"]} t={t} icon="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
              ...
              <Field label="Purpose of Request" name="purposeOfRequest" value={formData.purposeOfRequest} onChange={handleFieldChange} t={t} placeholder="E.g., Scholarship" required icon="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
              <Field label="Specific Purpose (Optional)" name="specificPurpose" value={formData.specificPurpose} onChange={handleFieldChange} t={t} placeholder="Optional details" icon="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2v10z" />
            </div>
          </>
        );
      case 3:
        return (
          <>
            ...
            {uploadedFile ? (
              ...
              <div className={`border-2 border-dashed ${t.cardBorder} rounded-xl p-6 text-center cursor-pointer hover:border-indigo-400 transition-colors`} onClick={() => fileInputRef.current?.click()} onDragOver={(e) => e.preventDefault()} onDrop={handleDrop}>
                ...
                <p className={`font-kumbh text-sm ${t.subtleText} mb-2`}>Drag & drop your file here, or</p>
                <span className="bg-indigo-500 hover:bg-indigo-600 text-white font-kumbh text-sm px-4 py-2 rounded-lg transition-colors inline-block">Browse Files</span>
                <p className={`font-kumbh text-xs ${t.subtleText} mt-2`}>Accepted formats: JPG, PNG, PDF (max 5MB)</p>
              </div>
            )}
          </>
        );
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
              ...
              {Array.from({ length: totalSteps }, (_, i) => (
                <div
                  ...
                  className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${
                    i + 1 <= step ? "bg-indigo-500" : "bg-gray-300/30"
                  }`}
                />
              ))}
            </div>
        ...
        {showPreview && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
            <div className={`${t.cardBg} border ${t.cardBorder} rounded-2xl w-full max-w-2xl p-6 max-h-[85vh] overflow-y-auto shadow-2xl`}>
        ...
        {showSuccess && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
            <div className={`${t.cardBg} border ${t.cardBorder} rounded-2xl w-full max-w-md p-8 text-center shadow-2xl`}>
              ...
              <p className={`font-kumbh text-sm ${t.subtleText} mt-2`}>Your COI request has been submitted successfully. You will be notified once it is ready for pickup.</p>
              <button onClick={handleSuccessContinue} className="mt-6 px-6 py-2.5 rounded-lg bg-green-500 hover:bg-green-600 text-white font-kumbh text-sm font-medium transition-colors">Continue</button>
            </div>
          </div>
        )}
      </div>
  ...
  <div>
    ...
    <div className="relative">
      ...
      <input
        ...
        className={`w-full border ${t.cardBorder} rounded-lg ${icon ? "pl-9 pr-3" : "px-3"} py-2.5 text-sm font-kumbh ${t.inputBg} ${t.inputText} focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 outline-none transition-all`}
      />
    </div>
  </div>
);
...
const SelectField = ({ label, options, name, value, onChange, t, icon }) => (
  <div>
    ...
    <div className="relative">
      ...
      <select name={name} value={value} onChange={onChange} className={`w-full border ${t.cardBorder} rounded-lg ${icon ? "pl-9 pr-3" : "px-3"} py-2.5 text-sm font-kumbh ${t.inputBg} ${t.inputText} focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 outline-none transition-all`}>
