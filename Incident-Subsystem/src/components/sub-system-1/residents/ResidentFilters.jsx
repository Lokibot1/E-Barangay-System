        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div className="group w-full xl:max-w-xl">
            <div className="relative w-full">
              ...
              <input
                type="text"
                placeholder="Search resident name or barangay ID..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className={`w-full rounded-[18px] border ${t.inputBorder} ${t.inputBg} ${t.inputText} pl-12 pr-11 py-3.5 text-sm font-normal placeholder:font-normal outline-none transition-all focus:ring-4 ${accent.ring} ${accent.border} ${
          ...
          <div className="grid w-full gap-3 sm:grid-cols-2 xl:w-auto xl:grid-cols-3">
            <FilterSelect label="Sector" value={categoryFilter} onChange={setCategoryFilter} options={SECTOR_OPTIONS} t={t} currentTheme={currentTheme} />
            <FilterSelect label="Purok" value={purokFilter} onChange={setPurokFilter} options={PUROK_OPTIONS} t={t} currentTheme={currentTheme} />
            <FilterSelect label="Residency" value={residencyFilter} onChange={setResidencyFilter} options={RESIDENCY_OPTIONS} t={t} currentTheme={currentTheme} />
          </div>
        </div>
      </div>
      ...
      <div className={`border-b px-5 py-4 sm:px-6 ${t.cardBorder} ${isDark ? 'bg-slate-950/70' : 'bg-white/70'}`}>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <span className={`text-sm font-semibold ${t.cardText} font-kumbh`}>
          ...
          {hasActiveFilters && (
            <button
              ...
              className="inline-flex items-center gap-1.5 text-sm font-semibold text-rose-500 transition-colors hover:text-rose-600 font-kumbh"
            >
              <FilterX size={15} />
              Reset Filters
            </button>
