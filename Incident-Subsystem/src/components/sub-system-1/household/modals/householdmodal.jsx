...
/**
 * @prop onToast - (toast) => void — parent provides addToast for error/success feedback
 */
const HouseholdModal = ({ isOpen, onClose, onEdit, data, t, currentTheme = 'modern', onToast }) => {
  ...
  const openResident = async (residentId) => {
    ...
    try {
      ...
      onToast?.({
        type: 'error',
        title: 'Load Failed',
        message: err.response?.data?.message || 'Failed to load resident profile.',
        duration: 4000,
      });
    }
  };
  ...
  // Save handler — replaces alert() with onToast
