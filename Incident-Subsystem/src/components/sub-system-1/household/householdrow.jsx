...
const HouseholdRow = ({
  ...
  const householdKey   = item.db_id ?? item.id;
  const isViewLoading  = disableActions && String(actionLoadingId) === String(householdKey) && actionLoadingMode === 'view';
  const isEditLoading  = disableActions && String(actionLoadingId) === String(householdKey) && actionLoadingMode === 'edit';
        ...
        <td className={`${cellBase} text-center`}>
          <div className="flex justify-center gap-2">
            <button
              ...
              className={`rounded-[18px] border ${t.cardBorder} px-4 py-3 text-slate-500 transition-all shadow-[0_10px_20px_rgba(15,23,42,0.06)] active:scale-90 disabled:cursor-wait disabled:opacity-70 ${accent.buttonHover}`}
            >
            ...
            {onEdit && (
              <button
                ...
                className={`rounded-[18px] border ${t.cardBorder} px-4 py-3 text-slate-500 transition-all shadow-[0_10px_20px_rgba(15,23,42,0.06)] active:scale-90 ${
                  disableActions ? 'cursor-wait opacity-70 ' : ''
                }${
                  isDark
              ...
              >
                {isEditLoading ? <Loader2 size={16} className="animate-spin" /> : <Edit2 size={16} />}
              </button>
            )}
            ...
            {onDeactivate && (
              <button
                ...
                disabled={disableActions}
                className="rounded-[18px] border border-rose-200 px-4 py-3 text-rose-400 transition-all shadow-[0_10px_20px_rgba(15,23,42,0.06)] active:scale-90 hover:bg-rose-600 hover:text-white hover:border-rose-600 disabled:cursor-not-allowed disabled:opacity-50"
              >
                <Trash2 size={16} />
              </button>
            )}
